<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">My Ships</h2>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if($ships->isEmpty())
                <div class="text-center py-16 bg-white rounded-xl shadow-sm border border-gray-100">
                    ...
                    <h3 class="mt-4 text-lg font-medium text-gray-700">No ships yet</h3>
                    <p class="mt-1 text-sm text-gray-400">Add your first ship to start placing orders.</p>
                    <a href="{{ route('ships.create') }}" class="mt-4 inline-block px-5 py-2 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700">Add Ship</a>
                </div>
            @else
                    ...
                    @foreach($ships as $ship)
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-start justify-between">
                            ...
                            <div class="flex gap-2">
                                ...
                                <form method="POST" action="{{ route('ships.destroy', $ship) }}" onsubmit="return confirm('Remove this ship?')">
                                    @csrf @method('DELETE')
